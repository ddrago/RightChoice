...
def populate():
    ...
    school_subjects_computing_higher = [{'name': 'Computing Science', 'level': 'Higher', 'career_name':"Software Engineer",
                                        'subjectArea': 'Computing'},{'name': 'Mathematics', 'level': 'Higher', 'career_name':"Mathematician", 'subjectArea': 'Maths'}]
    ...
    school_subjects_computing_nat5 = [{'name': 'Compuitng Science', 'level': 'Nat 5', 'career_name':"Software Engineer", 'subjectArea': 'Computing'},
                                      {'name': 'Mathematics', 'level': 'Nat 5', 'career_name':"Mathematician", 'subjectArea': 'Maths'}]
    ...
    for d in Colleges:
        t = College.objects.get_or_create(name=d["name"], location=d["location"], details=d["details"], collegeImage=d["collegeImage"], linkToCollegeWebsite=d["linkToCollegeWebsite"])[0]
        t.save()
    for c in College.objects.all():
        print(f'-{c}')


    courses_city_of_glasgow_college = [{'courseID':1, 'name':'Computing Science HNC', 'collegeName': College.objects.get(name="City of Glasgow College"), 'career_name':Career.objects.filter(areaStudy="Computing Science"), 'subReqName':SchoolSubjects.objects.filter(name="Mathematics", level="Nat 5"), 'gradesReq': 'CC', 'subjectSuggestions':'Computing Science, Maths and Physics'},
                                    {'courseID': 2, 'name': 'Finance and Accounting HNC', 'collegeName': College.objects.get(name="City of Glasgow College"), 'career_name': Career.objects.filter(areaStudy="Mathematician"), 'subReqName':SchoolSubjects.objects.filter(name="Mathematics", level="Nat 5"), 'gradesReq': 'CC', 'subjectSuggestions':'Business, Maths and Economics'}]
    for d in courses_city_of_glasgow_college:
        ...
        t.careerName.set(d["career_name"])
        t.subReqName.set(d["subReqName"])
        t.save()
    ...
    for d in Universities:
        t = University.objects.get_or_create(name=d['name'], location=d['location'], details=d['details'], universityImage=d["universityImage"], linkToUniWebsite=d["linkToUniWebsite"])[0]
        t.save()
    ...
    courses_glasgow_uni = [{'courseID': 1, 'name': 'Computing Science Bsc', 'universityName':University.objects.get(name="Glasgow University"), 'career_name': Career.objects.filter(areaStudy="Computing Science"), 'subReqName':SchoolSubjects.objects.filter(name="Mathematics", level="Higher"), 'gradesReq': 'AAAB', 'subjectSuggestions':'Computing Science, Maths and Physics'},
                           {'courseID': 2, 'name': 'Finance and Accounting Bsc', 'universityName':University.objects.get(name="Glasgow University"), 'career_name': Career.objects.filter(areaStudy="Mathematician"), 'subReqName':SchoolSubjects.objects.filter(name="Mathematics", level="Higher"), 'gradesReq': 'AAAB', 'subjectSuggestions':'Business, Maths and Economics'}]

    print(Career.objects.filter(areaStudy="Mathematician"))
    ...
    company = [{'name': "Apprenticeship Scotland", 'location': "edinburgh", "details": "The leading provider of apprentieships in Scotland.", 'linkToCompanyWebsite': "jjjj", 'companyImage':'apprenticeship.png' }]

    for d in company:
        t = Company.objects.get_or_create(name=d["name"], location=d["location"], details=d["details"], linkToCompanyWebsite=d["linkToCompanyWebsite"], companyImage=d["companyImage"])[0]
        t.save()
    for c in Company.objects.all():
        print(f'-{c}')


    courses_apprenticeships_scotland = [{'courseID':1, 'name':'Accounting Apprenticeship', 'companyName': Company.objects.get(name='Apprenticeship Scotland'), 'careerName':Career.objects.filter(areaStudy="Mathematician"), 'gradesReq': 'No qualifications required', 'subjectSuggestions':'Business, Maths and Economics'}]
    ...
    print(Career.objects.get(areaStudy="Computing Science"))
